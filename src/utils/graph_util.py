...
def get_number_of_ally_nodes(graphs):
    if type(graphs) == dgl.BatchedDGLGraph:
...
def copy_graph(graph: dgl.DGLGraph):
    n_nodes = graph.number_of_nodes()
    nf = graph.ndata['node_feature']
    init_nf = graph.ndata['init_node_feature']
    node_type = graph.ndata['node_type']

    device = nf.device

    out_g = dgl.DGLGraph()
    out_g.set_n_initializer(minus_large_num_initializer)

    out_g.add_nodes(n_nodes, {'node_feature': torch.zeros_like(nf).to(device),
                              'node_type': node_type,
                              'init_node_feature': torch.zeros_like(init_nf).to(device)})

    self_edge_type = torch.Tensor(data=(EDGE_SELF,)).to(device)

    out_g.add_edges(range(n_nodes), range(n_nodes), {'edge_type': self_edge_type.repeat(n_nodes)})
